...
def save_image(image_numpy, image_path, aspect_ratio=1.0):
    ...
    Parameters:
        ...
        aspect_ratio (float)      -- aspect ratio of the image (default=1.0)
    """
...
def print_numpy(x, val=True, shp=False):
    ...
    Parameters:
        x (numpy array) -- the array to print the statistics of
    ...
    if val:
        ...
        print('mean = %3.3f, min = %3.3f, max = %3.3f, median = %3.3f, std=%3.3f'
              .format(np.mean(x), np.min(x), np.max(x), np.median(x), np.std(x)))
