...
class Pix2PixModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            # define a discriminator, conditional GANs need to feed both input and output to the discriminator,
            # so the input channel number is opt.input_nc + opt.output_nc.
    ...
    def set_input(self, model_input):
        """
        Unpack the input data from the dataloader and perform necessary pre-processing steps.
    ...
    def optimize_parameters(self):
        ...
        self.backward_dis()                         # calculate gradients for D
        self.optimizer_D.step()                     # update D's weights
        # update G
        self.set_requires_grad(self.netD, False)    # D requires no gradients when optimizing G
        self.optimizer_G.zero_grad()                # set G's gradients to zero
        self.backward_gen()                         # calculate gradients for G
