...
class CycleGANModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        For CycleGAN, in addition to GAN losses,
        we introduce lambda_A, lambda_B, and lambda_identity for the following losses.
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_identity', type=float, default=0.5, help='use identity mapping. Setting '
                                                                                   'lambda_identity other than 0 has '
                                                                                   'an effect of scaling the weight '
                                                                                   'of the identity mapping loss. For '
                                                                                   'example, if the weight of the '
                                                                                   'identity loss should be 10 times '
                                                                                   'smaller than the weight of the '
                                                                                   'reconstruction loss, please set '
                                                                                   'lambda_identity = 0.1')
    ...
    def set_input(self, model_input):
        ...
        self.real_A = model_input['A' if a_to_b else 'B'].to(self.device)
        self.real_B = model_input['B' if a_to_b else 'A'].to(self.device)
        self.image_paths = model_input['A_paths' if a_to_b else 'B_paths']
    ...
    def backward_dis_basic(self, net_d, real, fake):
        """
        ...
        We also call loss_dis.backward() to calculate the gradients.
        """
        # Real
    ...
    def backward_dis_a(self):
        ...
        self.loss_D_A = self.backward_dis_basic(self.netD_A, self.real_B, fake_b)

    def backward_dis_b(self):
        """Calculate GAN loss for discriminator D_B"""
        fake_a = self.fake_A_pool.query(self.fake_A)
        self.loss_D_B = self.backward_dis_basic(self.netD_B, self.real_A, fake_a)
    ...
    def optimize_parameters(self):
        ...
        self.backward_gen()             # calculate gradients for G_A and G_B
        self.optimizer_G.step()         # update G_A and G_B's weights
        # D_A and D_B
        self.set_requires_grad([self.netD_A, self.netD_B], True)
        self.optimizer_D.zero_grad()    # set D_A and D_B's gradients to zero
        self.backward_dis_a()           # calculate gradients for D_A
        self.backward_dis_b()           # calculate gradients for D_B
